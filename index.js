...
function getWinnersByYear(data, getYearsCB, getWinnersCB /* code here */) {
  let winnerYears = [];
...
function getAverageGoals(data /* code here */) {
  ...
  return `${((averageHomeGoals + averageAwayGoals) / data.length).toFixed(2)}`;
  //   return `${averageHomeGoals / data.length + averageAwayGoals / data.length}`; // <= confused on why this didnt work
  //
}
